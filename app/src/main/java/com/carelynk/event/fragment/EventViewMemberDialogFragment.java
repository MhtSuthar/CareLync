...
public class EventViewMemberDialogFragment extends DialogFragment {

    private DialogCommentListBinding binding;
    private static final String TAG = "EventDetailDialogFragme";
    private ViewMemberEventListAdapter viewMemberEventListAdapter;
    ...
    void getViewMember(){
        if(AppUtils.isOnline(getActivity())){
                    ...
                    try {
                        if(result != null) {
                            ...
                            for (int i = 0; i < jsonArray.length(); i++) {
                                ...
                                hashMap.put("user_id", obj.getString("UserId"));
                                //hashMap.put("ProfilePicUrl", obj.getString("ProfilePicUrl"));
                                mList.add(hashMap);
                            }
            ...
            asyncTaskGetCommon.execute(ApiFactory.API_BASE_URL+""+ Urls.VIEW_EVENT_USER+""+ getArguments().getString(Constants.EXTRA_EVENT_ID));
        }else{
    ...
    private void setRecyclerAdapter(List<HashMap<String, String>> mList) {
        viewMemberEventListAdapter = new ViewMemberEventListAdapter(getActivity(), mList, EventViewMemberDialogFragment.this);
        binding.recyclerView.setHasFixedSize(true);
        LinearLayoutManager mLayoutManager = new LinearLayoutManager(getActivity());
        binding.recyclerView.setLayoutManager(mLayoutManager);
        binding.recyclerView.setItemAnimator(new DefaultItemAnimator());
        binding.recyclerView.setAdapter(viewMemberEventListAdapter);
    }

    private void initToolbar() {
        binding.includeToolbar.toolbarTitle.setText("View Members");
